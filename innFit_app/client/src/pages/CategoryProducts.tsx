...
export default function CategoryProducts({ category, onBack, onSelectProduct }: CategoryProductsProps) {
  ...
  const filtered = products.filter(p => {
    const matchesCategory = !category || category === "all" || p.category === category.toLowerCase();
  ...
  return (
    <main className="relative min-h-screen w-full flex flex-col bg-white safe-top">
      <header className="container-responsive pt-4 pb-2 flex items-center gap-4 border-b">
        <button
          onClick={onBack}
        ...
        <h1 className="font-['Montserrat',Helvetica] font-semibold text-lg text-black">{getCategoryTitle()}</h1>
      </header>
      ...
      <div className="container-responsive pb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1 bg-[#f5f5f5] rounded-md text-xs font-['Montserrat',Helvetica] flex items-center gap-1 hover:bg-gray-300 active:scale-95">
              Sort
            ...
            >
              ...
              <div className="p-3">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    addToCart(product.id, 1, "M", "#000000", product.price);
                    setToastMessage(`${product.name} added to cart 🛒`);
                    setShowToast(true);
                    setTimeout(() => setShowToast(false), 2000);
                  }}
                  className="w-full h-7 bg-[#0088ff] hover:bg-[#0077dd] text-white text-xs rounded font-semibold active:scale-95 transition-all"
                >
                  Add to Cart
                </button>
              </div>
